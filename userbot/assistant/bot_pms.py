    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"مرحبا عزيزي ! ⚕️ {mention},\
                        ...
                        \n\nتم التطوير بواسطة  [جـمثون](https://t.me/ICE16)"
        buttons = [
            (
                Button.url("الدعم", "https://T.me/ICE16"),
                Button.url(
                    "للـتنصيب",
                    "https://T.ME/ICE16",
                ),
...
@ICE16.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await ICE16.tgbot.send_message(
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await ICE16.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await ICE16.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await ICE16.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await ICE16.tgbot.get_entity(BOTLOG_CHATID)
            await ICE16.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await ICE16.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@ICE16.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await ICE16.get_entity(user_id)
    except Exception as e:
...
@ICE16.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@ICE16.bot_cmd(incoming=True, func=lambda e: e.is_private)
@ICE16.bot_cmd(edited=True, func=lambda e: e.is_private)
