...
# ترجمه وكتابة فريق آيس
@ICE16.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@ICE16.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
# ترجمه وكتابة فريق آيس
@ICE16.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
# ترجمه وكتابة فريق آيس
@ICE16.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@ICE16.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌔︙ - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
# ترجمه وكتابة فريق آيس
@ICE16.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌔︙ - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
@ICE16.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق آيس
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def pmpermit_on(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "off":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق آيس
