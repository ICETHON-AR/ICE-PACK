...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await ICE16.send_message(
                BOTLOG_CHATID,
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await ICE16(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await ICE16(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
