...
# copyright for ICE16 © 2021
@ICE16.ar_cmd(incoming=True, groups_only=True)
...
async def _(event):
    ...
    await edit_or_reply(
        event,
        "⌔︙ تم اضافة {} الكلمة في قائمة المنع بنجاح".format(len(to_blacklist)),
    )
...
async def _(event):
    ...
    await edit_or_reply(
        event,
        f"⌔︙ تم ازالة الكلمة {successful} / {len(to_unblacklist)} من قائمة المنع بنجاح",
    )
